...
exports.getUserprofile = async (request, response) => {
	Userprofile.findOne({ where: { userId: request.params.id } })
		.then((user) => {
			if (user === null) {
...
exports.createorupdate = async function (req, res) {
	try {
		Userprofile.findOne({ where: { userId: req.params.id } }).then(
			(user) => {
				if (user !== null) {
					const userprofileId = user.id;
					const picture = req.file?.path ? req.file.path : '';
					const { gender, phonenumber, bio } = req.body;
					return Userprofile.findByPk(userprofileId)
						.then((userprofile) => {
							userprofile
								.update({
									gender:
								...
								.then((updatedUserprofile) => {
									res.status(200).send({
										...
										data: {
											...
											phonenumber:
												phonenumber ||
												updatedUserprofile.phonenumber,
											bio: bio || updatedUserprofile.bio,
											picture:
												picture ||
												updatedUserprofile.picture,
										},
									});
								})
...
exports.delete = async function (req, res) {
	try {
		return Userprofile.findByPk(req.params.userprofileId)
			.then((userprofile) => {
				if (!userprofile) {
