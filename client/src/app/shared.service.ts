...
export class SharedService implements OnInit {
  ...
  // categoryId = new Subject<any>();
  CommentList = new BehaviorSubject({});
  cList = new BehaviorSubject<any>([]);
  categoryId = new BehaviorSubject(0);
  userName = new BehaviorSubject('Not login');
  ...
  loginFlag = new BehaviorSubject(false);
  isLoggedIn!: Boolean;
  username: any = '';
  loginUserId: any;
  token: any;
  ...
  createUserProfile(formData: any) {
    let id = JSON.parse(localStorage.getItem('currentUser')!).id;
  ...
  postLike(id: any, postId: any) {
    return this.http.post(`like/${id}/${postId}`, '');
  }
  getLikeByPostId(id: any) {
    return this.http.get(`like/post/${id}`);
  }

  updateComments(id: number, formData: any) {
  ...
  createComments(userId: any, postId: any, formData: any) {
  ...
  upDatePost(formData: any, id: any) {
    this.http
      .put(`http://localhost:8000/post/${id}/update`, formData, {
        headers: {
          Authorization:
            'Bearer ' +
            JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
  ...
  createPosts(formValues: any) {
    ...
    this.http
      .post('http://localhost:8000/post/38/9/create', formValues, {
        headers: {
          Authorization:
            'Bearer ' +
            JSON.parse(localStorage.getItem('currentUser')!).accesstoken,
  ...
  getLogin(values: any) {
    ...
    this.http.post(`user/login`, values).subscribe(
      (res) => {
        // console.log(res);
  ...
  getPostOnCategorySelect(id: any) {
